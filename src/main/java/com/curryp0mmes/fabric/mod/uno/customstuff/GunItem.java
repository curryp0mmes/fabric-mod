...
public class GunItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (ammo == -15 && !user.abilities.creativeMode) {
            ...
            if (world.isClient && !user.abilities.creativeMode) {

                System.out.println(ammo + 15);
                ammo--;
                tag.putInt("ammunition", ammo);
                itemStack.setTag(tag);
                user.sendMessage(new TranslatableText("item.curry.pistol.ammo_left").append(String.valueOf(ammo)), true);
            }
